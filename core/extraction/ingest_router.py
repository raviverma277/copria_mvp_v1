...
def select_source() -> IngestSource:
    val = (
        os.environ.get("INGEST_SOURCE")
        or os.environ.get("COPRIA_INGEST_SOURCE")
        or "local"
    ).lower()
    try:
...
def from_local(
    parsed_sov, parsed_loss, notes, attachments, run_id: str
) -> SubmissionBundle:
...
def get_bundle_auto(local_args: dict, run_id: str) -> SubmissionBundle:
    ...
    """
    has_local = any(
        local_args.get(k) for k in ("parsed_sov", "parsed_loss", "attachments")
    )
    return (
        from_local(**local_args, run_id=run_id)
        if has_local
        else from_cytora(run_id=run_id)
    )
