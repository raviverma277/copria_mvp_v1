...
def get_llm_status():
    ...
    model = (
        _last_model
        or os.environ.get("LLM_MINER_MODEL")
    ...
    try:
        ...
        api_key_set = bool(
            os.environ.get("OPENAI_API_KEY") or st.secrets.get("OPENAI_API_KEY", "")
        )
    ...
    if _last_start_ts:
        try:
            ...
            last_call_iso = (
                datetime.fromtimestamp(_last_start_ts, tz=timezone.utc)
                .astimezone()
    ...
    return {
        ...
        "last_duration": (
            None if _last_duration_s is None else round(_last_duration_s, 2)
        ),
